 ...
 * - 400: Bad Request Exception -> 잘못된 요청 (사용자가 지정 형식으로 입력하지 않은 경우)
 * - 403: Forbidden -> 접근 권한 없음 (로그인 실패 등)
 * - 404: Page Not Found -> 페이지를 찾을 수 없음 (웹 브라우저상 주소가 잘못된 경우)
 * - 500: Server Error -> 백엔드 프로그램이 겪는 Runtime Error (백엔드 문제)
 * 
 * 사용자가 입력값 형식을 지키도록 강제하는 것은 백엔드에겐 보안과 연결되는 중요 사안
 * 사용자 입력값을 검사해 지정된 형식이 아닐 경우 적절한 예외처리를 수행해야 한다
 */

const multer = require("multer");

class Exception extends Error {
    // HTTP 상태 코드를 의미하는 멤버 변수
    #code;
    #name;
